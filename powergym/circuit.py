...
class Circuits():
    ...
    def initialize(self, noWei=True):
        ...
        if self.batt_file != '':
            self._gen_bat_obj()   #电池battery的初始化是在这个函数里面
    ...
    def set_all_capacitor_statuses(self, statuses, change_dss=True):
        ...
        for i, cap in enumerate(self.capacitors.keys()):
            capa = self.capacitors[cap] #这一个是获取当前的capacitor状态
            diff[i] = abs(capa.status - statuses[i])
            capa.status = statuses[i]
            cap2st[capa.name[10:]] = statuses[i]  #设置电容器状态
        ...
        if change_dss:
            ...
            while True:
                if dssCap.Name in cap2st:
                    dssCap.States = [cap2st[dssCap.Name]]   #这一段是将电容器状态写到dss里面
                if dssCap.Next==0: break
        return diff
    ...
    def set_all_regulator_tappings(self, tapnums, change_dss=True):
        ...
        taps = tapnums*step + mintap   #调整成当前的tap
    ...
    def set_all_batteries_before_solve(self, nkws_or_states, change_dss=True):
        ...
        for i, bat in enumerate(self.batteries.keys()):
            ...
            kw = batt.state_projection(nkws_or_states[i]) # projection  #将battery的输出转变成功率
            kvar = kw / batt.pf
        ...
        if change_dss:
            ...
            while True:
                if dssGen.Name in bat2kwkvar:
                    ...
                    dssGen.kW = kw
                    dssGen.kvar = kvar   #将电池的功率输入到电网中，与电网进行交互
    ...
    def set_all_batteries_after_solve(self):
        ...
        return soc_errs, discharge_errs #计算的是电池与最初始时候的相对差
    ...
    def _gen_bat_obj(self):
        ...
        while True:
            ...
            if name in batts:
                ...
                # if not specifying the phases, use all phases at the bus
                    phases = self.bus_phase[bus]
                self.add_batteries('Battery.'+name, bus, phases, feature)  #在这里将battery初始化加进去
            if dssGen.Next==0: break
...
class Battery(Node):
    def __init__(self, dss, batname, bus1, phases, feature, bat_act_num=33):
        ...
        self.dss = dss                     # the circuit's dss simulator object
        self.max_kw = feature.max_kw       # maximum power magnitude 最大功率
        self.pf = feature.pf               # power factor 这个feature是存储在csv文件里表示电池状态的量
        self.max_kwh = feature.max_kwh     # capacity 最大储能
        self.kwh = feature.initial_kwh     # current charge 初始储能量
        self.soc = self.kwh / self.max_kwh # state of charge 当前储能的百分比
        ...
        if bat_act_num == np.inf:
            ...
            diff = self.max_kw / mode_num  #每次蓄电池调整的功率变化量
            ## avail_kw: a discrete range from -max_kw to max_kw
            self.avail_kw = [n*diff for n in range(-mode_num, mode_num+1)] #可供调整的功率
    ...
    def state_projection(self, nkw_or_state):
        ...
        if self.bat_act_num == np.inf:
            ...
            mid = len(self.avail_kw)//2  #这个中间状态是用来区分电池是充电还是放电的，
            if state > mid: # discharging  当电池state大于中间状态时，是放电的
                ...
                allowed_kw = (self.kwh - self.max_kwh)/self.duration # min kw  允许充电的功，负数表示有空间可以充电
                if self.avail_kw[state] < allowed_kw: #如果电池的放电功率是小于当前电池的可充电空间的
                    state = int(state + np.ceil((allowed_kw-self.avail_kw[state])/ \
                               (self.avail_kw[1]-self.avail_kw[0])-1e-8) ) #在一个小时内，电池充了可充电空间-充电功率的攻（充电功率负表示放电了）
            self.state = state # 除以每一份功率间隔就是电池的状态
            return self.avail_kw[state]  #返回当前可用的充电功率
    ...
    def actual_power(self):  #这个函数返回电池的真实功率，是从dss文件里读处理的真实功率
