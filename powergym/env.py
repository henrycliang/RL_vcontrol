...
class Env(gym.Env):
    ...
    def step(self, action):
        ...
        if self.cap_num>0:
            ...
            action_idx += self.cap_num   #完成电容器对应的action，所以action_idx加
        ...
        self.circuit.dss.ActiveCircuit.Solution.Solve()   #在以上3种control device设置完之后，solve power flow

        ### update battery kWh. record soc_err and discharge_err
        if self.bat_num>0:  # 这一段是更新电池的状态了
        ...
        for bus_name in self.all_bus_names:
            bus_voltages[bus_name] = self.circuit.bus_voltage(bus_name)#返回的是当前bus_name上的a相电压赋值，相角，b相上的电压幅值，相角......等，所以是一个长度为6的数组
            bus_voltages[bus_name] = [bus_voltages[bus_name][i] for i in range(len(bus_voltages[bus_name])) if i%2==0]
        
        self.obs['bus_voltages'] = bus_voltages  #更新状态变量，最主要是这个voltage
        self.obs['cap_statuses'] = cap_statuses
        self.obs['reg_statuses'] = reg_statuses
        self.obs['bat_statuses'] = bat_statuses
        self.obs['power_loss'] = - self.circuit.total_loss()[0]/self.circuit.total_power()[0]  #根据论文中(6)式更新power loss
        ...
        if not done:
            ...
            self.reward_recorder.append(self.episode_reward)
            self.episode_reward=0
    ...
    def reset(self, load_profile_idx=0):
        ...
        """
        ## Let the reset choose the load profile randomly
        # load_profile_idx = np.random.randint(0, 73)
        ## During testing, we should remove it!
    ...
    def plot_graph(self, node_bound='minimum', 
        ...
        if show_voltages:
            ...
            self.voltage_mat.append(voltages)
            self.bus_name_list=nodes
        else:
            nx.draw_networkx_nodes(graph, pos, nodelist=nodes, node_color=np.ones(len(voltages))*0.985, vmin=vmin, vmax=vmax, cmap=cmap, node_size=node_size,alpha=0.8)

        if show_node_labels:
            node_labels = {node:node for node in pos}
            nx.draw_networkx_labels(graph, pos, labels= node_labels, font_size=30)

        # show source bus
        loc={self.source_bus:(pos[self.source_bus][0]+shift, pos[self.source_bus][1]-shift)}
        nx.draw_networkx_labels(graph, loc, labels={self.source_bus:'src'}, font_size=30)
        ...
        if show_controllers:
            if self.cap_num>0:
                ...
                nx.draw_networkx_labels(graph, loc, labels=labels, font_size=35,
        ...
        if show_actions:
            ...
            plt.text(text_loc_x, text_loc_y, s='Time Step='+str(self.t)+':00', fontsize=32)
